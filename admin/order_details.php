...
$pageurl = new moodle_url('/local/order/admin/order_details.php',
                          array('orderid' => $orderid,
                                'return'  => $return));
$PAGE->set_url($pageurl);

$PAGE->set_title(get_string('title_admin_order_details', 'local_order'));
$PAGE->set_heading(get_string('heading_admin_order_details', 'local_order'));

$userlink = new moodle_url('/user/view.php',
                           array('id' => $order->entry->userid,
                                 'course' => SITEID));

echo $OUTPUT->header();
echo $OUTPUT->box_start('', 'local_order');
echo $OUTPUT->heading(get_string('heading_admin_order_details', 'local_order'));
...
<table>
    ...
    <tr>
        <td class='label'><?php print_string('col_fullname', 'local_order'); ?></td>
        <td class='value'>
            <a href="<?php echo $userlink; ?>" class="userlink">
                <?php echo fullname($order->user); ?>
            </a>
        </td>
    </tr>
    <tr>
...
<?php
echo $OUTPUT->box_end();
echo $OUTPUT->heading(get_string('heading_admin_order_items', 'local_order'));
...
if ($order->entry->status == local_order::STATUS_PENDING) {
    $output .= html_writer::link($vlink, get_string('link_admin_order_validate', 'local_order'), array('class' => 'validatelink'));
}
...
if ( ($order->entry->status != local_order::STATUS_PAID) &&
    ...
    $output .= html_writer::link($clink, get_string('link_admin_order_cancel', 'local_order'), array('class' => 'cancellink'));
}
